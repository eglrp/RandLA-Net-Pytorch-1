...
class Building_block(nn.Module):
    ...
    def forward(self, xyz, feature,
        ...
        # print('1', f_neighbours.shape)
        # print('2', f_xyz.shape)
        f_concat = torch.cat([f_neighbours, f_xyz], dim=1)
        # print('3', f_concat.shape)
    ...
    def encoder_decoder(self, features, xyz, neigh_idx, sub_idx, interp_idx):
        ...
        for j in range(self.config.num_layers):
            f_interp_i = self.nearest_interpolation(features,
                                                    interp_idx[-j - 1])
            # print('1_', f_encoder_list[-j - 2].shape)
            # print('2_', f_interp_i.shape)
            # print(torch.cat([f_encoder_list[-j - 2], f_interp_i], dim=1).shape)
