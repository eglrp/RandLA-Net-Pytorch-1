...
__global__ void cuComputeDistanceGlobal(float* A, int wA, float* B, int wB,
  ...
  int cond1 =
      (begin_B + tx < wB);  // used to write in shared memory & to computations
                            // and to write in output matrix
...
/**
 * Gathers k-th smallest distances for each column of the distance matrix in the
 * top.
 *
 ...
 */
__global__ void cuInsertionSort(float* dist, long* ind, int width, int height,
                                int k) {
  // Variables
  int l, i, j;
  float* p_dist;
...
void debug(float* dist_dev, long* ind_dev, const int query_nb, const int k) {
  ...
  cudaMemcpy(dist_host, dist_dev, query_nb * k * sizeof(float),
             cudaMemcpyDeviceToHost);

  cudaMemcpy(idx_host, ind_dev, query_nb * k * sizeof(long),
             cudaMemcpyDeviceToHost);
...
void knn_device(float* ref_dev, int ref_nb, float* query_dev, int query_nb,
                int dim, int k, float* dist_dev, long* ind_dev,
                cudaStream_t stream) {
  // Grids ans threads
  ...
  cuComputeDistanceGlobal<<<g_16x16, t_16x16, 0, stream>>>(
      ref_dev, ref_nb, query_dev, query_nb, dim, dist_dev);
  ...
  cuInsertionSort<<<g_256x1, t_256x1, 0, stream>>>(dist_dev, ind_dev, query_nb,
                                                   ref_nb, k);
